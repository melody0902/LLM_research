...
class UnigramUtils:
    ...
    def __init__(self, config: UnigramConfig, *args, **kwargs) -> None:
        ...
        green_size = int(self.config.gamma * self.config.vocab_size)
        self.mask = np.array(
            [True] * green_size + [False] * (self.config.vocab_size - green_size),
            dtype=bool
        )
    ...
    def _hash_fn(x: int) -> int:
        """Hash function to generate random seed."""
        x = np.int64(x)
    ...
    def score_sequence(self, input_ids: torch.Tensor) -> tuple:
        ...
        for idx in range(num_tokens_scored):
            curr_token = input_ids[idx].item()
            if curr_token < len(self.mask) and self.mask[curr_token]:
                green_token_count += 1
...
class UnigramLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, config: UnigramConfig, utils: UnigramUtils, *args, **kwargs):
        ...
        # 用 bool mask 比較自然
        self.green_list_mask = torch.tensor(self.utils.mask, dtype=torch.bool)
    ...
    def _get_aligned_greenlist_mask(
        self,
        vocab_size: int,
        device: torch.device
    ) -> torch.Tensor:
        """
        Align green list mask to current logits vocab size.

        This fixes cases where tokenizer length and model output vocab size differ
        (e.g., some Qwen models).
        """
        mask = self.green_list_mask.to(device)

        if mask.numel() == vocab_size:
            return mask

        fixed_mask = torch.zeros(vocab_size, dtype=torch.bool, device=device)
        n = min(mask.numel(), vocab_size)
        fixed_mask[:n] = mask[:n]
    ...
    def __call__(
        ...
        aligned_mask = self._get_aligned_greenlist_mask(
            vocab_size=vocab_size,
            device=scores.device
        )

        greenlist_mask = aligned_mask.unsqueeze(0).expand(batch_size, -1)

        scores = self._bias_greenlist_logits(
            scores=scores,
            greenlist_mask=greenlist_mask,
...
class Unigram(BaseWatermark):
    ...
    def __init__(
        self,
        algorithm_config: str | UnigramConfig,
