...
class KGWUtils:
    ...
    def score_sequence(self, input_ids: torch.Tensor) -> tuple[float, list[int]]:
        ...
        # 範例：根據實際是否落在 greenlist 判斷
        green_token_flags = []
        for idx in range(self.config.prefix_length):
            curr_token = input_ids[idx]
            greenlist_ids = self.get_greenlist_ids(input_ids[:idx]) if idx > 0 else []
...
class KGWLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # print(f"[DEBUG] Applying watermark bias, delta={self.config.delta}") 
        return scores
...
class KGW(BaseWatermark):
    ...
    def get_data_for_visualization(self, text: str, *args, **kwargs) -> DataForVisualization:
        ...
        decoded_tokens = [
            self.config.generation_tokenizer.decode([tid.item()], clean_up_tokenization_spaces=False)
            for tid in encoded_text
        ]
        token_ids = [tid.item() for tid in encoded_text]

        dfv = DataForVisualization(decoded_tokens, highlight_values, token_ids)
        
        dfv.token_ids = token_ids
