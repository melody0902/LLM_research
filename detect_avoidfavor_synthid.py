...
OUTPUT_PATH = (
    "outputs/test/detect_avoid0517/"
    "synthid_subset_vs_baseline_{domain}_{model_tag}_{text_key}_top{top_k}.json"
).format(
    domain=DOMAIN,
    model_tag=REWRITE_MODEL_TAG,
    text_key=WM_TEXT_KEY,
    top_k=TOP_K_TOKENS,
)
...
#    支援 gt / lt，避免 SynthID 分數方向跟別的 detector 不一致
# ============================================================
...
def main():
    ...
    rewrite_path = REWRITE_AVOID_JSON.format(
        domain=DOMAIN,
        alg=ALGORITHM,
        model_tag=REWRITE_MODEL_TAG,
    )
    ...
    token_subset = load_token_subset(
        TOKEN_SET_TMPL.format(domain=DOMAIN, alg=ALGORITHM),
        TOP_K_TOKENS
    )
    ...
    for w, p in tqdm(zip(wm_texts[:n], plain_texts[:n]), total=n):
