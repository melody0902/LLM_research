...
# 這邊是新增的
from watermark.signature import SignatureSetCollector

def generate_signature_set(algorithm_name: str, dataset_path: str, max_samples: int, output_path: str):
    print(f"🔍 開始收集 signature set：{algorithm_name} from {dataset_path}")
    ...
    if 'zhtw' in dataset_path.lower():
        ...
        raise ValueError(f"不支援的資料集：{dataset_path}")

    # 4. 建立 collector
    collector = SignatureSetCollector(watermark)

    for idx in range(len(dataset)):
        sample = dataset[idx]
        text = sample['text'] if isinstance(sample, dict) and 'text' in sample else str(sample)
        collector.collect_from_text(text)
        print(f"已處理樣本 {idx+1}/{len(dataset)}")

    # 5. 儲存 signature set
    collector.save_signature_set(output_path)
