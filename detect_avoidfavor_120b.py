# ============================================================
# detect_avoidfavor_120b.py
#
# 120B detection for KGW / SWEET / Unigram / EXP.
...
DEFAULT_ALGORITHMS = ["KGW", "SWEET", "Unigram", "EXP"]
...
def get_rewrite_avoid_path(domain, algorithm, model_name, sample_mode, sample_seed, max_samples, top_k):
...
def get_token_path(domain, algorithm, model_name, sample_mode, sample_seed, max_samples):
    safe = sanitize_model_name(model_name)
    return os.path.join(
        TOKEN_DIR,
...
def get_output_path(domain, model_name, sample_mode, sample_seed, max_samples, top_k, tag=None):
    ...
    return os.path.join(
        OUTPUT_DIR,
        f"detect_avoidfavor_120b_{domain}_{safe}_{sample_mode}_seed{sample_seed}_n{max_samples}_top{top_k}{suffix}.json",
    )
...
class SubsetAwareKGWDetector:
    def __init__(self, wm, token_subset):
        self.wm = wm
        self.utils = wm.utils
...
class SubsetAwareSWEETDetector(SubsetAwareKGWDetector):
    def __init__(self, wm, token_subset):
        super().__init__(wm, token_subset)
...
def baseline_detect_score(wm, text):
...
def run_domain(args, cfg, domain):
    ...
    for alg in args.algorithms:
        if alg == "SynthID":
            print("[SKIP] SynthID should be run with detect_avoidfavor_synthid_120b.py")
            continue
        ...
        if n == 0:
            print(f"[SKIP] no valid texts for {domain}/{alg}")
            continue
        ...
        for w, p in tqdm(zip(wm_texts, plain_texts), total=n):
            wm_base.append(baseline_detect_score(wm, w))
            plain_base.append(baseline_detect_score(wm, p))
        ...
        results[alg] = {
            ...
            "baseline_wm_avoid_vs_unwm_favor": find_best_threshold_both(wm_base, plain_base),
            "subset_wm_avoid_vs_unwm_favor": find_best_threshold_both(wm_subset, plain_subset),
            "raw_scores": {
                "baseline_positive": wm_base,
                "baseline_negative": plain_base,
                "subset_positive": wm_subset,
                "subset_negative": plain_subset,
